    ...
    async fetchCoinData(id){
      ...
      "July", "August", "September", "October", "November", "December"
            ...
            data.prices.forEach((item) => {
              ...
              let priceDate = new Date(item[0])
              xData.push(`${monthNames[priceDate.getMonth()]} ${priceDate.getDate()}`)
            })
        ...
        if(this.state.coin.id === id) {
            return (
                  ...
                  { this.state.options && this.state.series
                    ?
                    <div>
                      <h1 style={{marginBottom: '30px'}}>{this.state.coin.name} over the past 7 days</h1>
                      <Chart
